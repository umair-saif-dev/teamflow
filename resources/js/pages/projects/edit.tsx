...
export default function EditProject({ project, members }: { project: { data: Project }; members: Member[] }) {
    const { data, setData, put, processing, errors } = useForm({
        name: project.data.name,
        description: project.data.description ?? '',
        member_ids: project.data.members.map((member) => member.id),
    });

    return (
        <AppLayout breadcrumbs={[{ title: 'Projects', href: '/projects' }, { title: project.data.name, href: `/projects/${project.data.id}` }, { title: 'Edit', href: `/projects/${project.data.id}/edit` }]}>
            <Head title={`Edit ${project.data.name}`} />
            <form
                className="max-w-2xl space-y-4 p-4"
                onSubmit={(event) => {
                    event.preventDefault();
                    put(`/projects/${project.data.id}`);
                }}
            >
